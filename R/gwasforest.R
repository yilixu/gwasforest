...
#' @param customFilename_studyName string, relative or full path to the study name file, if users want to use their own study names which is not standardized (see "stdColnames"); all study names should be in one column with a header; also, study names should be in the order of that they first appear in the input data columns.
#' @param keepStudyOrder logical, whether to keep studies (except for meta study) in the original order provided by user (from customFilename_studyName), or sort them alphabatically on the combined forest plot; meta study will always be put at the bottom of the combined forest plot.
